...
fun ApiUserInfo.toUserInfo(): UserInfo{
    return UserInfo(quiz_results = this.quiz_results.map { it.toQuizResults() }, likesLessons = likes_info_lessons_ids, userID = user_id)
}
...
fun UserInfo.toApiUserInfo(): ApiUserInfo{
    return ApiUserInfo(quiz_results = this.quiz_results.map { it.toApiQuizResults() }, likesLessons, userID)
}
